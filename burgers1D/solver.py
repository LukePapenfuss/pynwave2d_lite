...
import burgers1D.burgers as burgers
import burgers1D.burgutils as butil
from nwave import *
...
def main(parfile):
    ...
    D1, D2 = butil.init_derivative_operators(x, params) # Gets the derivative operators from the parameter file
    ...
    eqs = burgers.Burgers(1, g) # Creates the equation (1: Number of PDEs in system, g: grid)
    ...
    fname = f"{output_dir}/burgers1D_0000.curve"
    ...
    for i in range(1, Nt + 1): # Loop through all time steps
        ...
        if i % output_interval == 0: # Check if we are supposed to output on this step
            ...
            fname = f"{output_dir}/burgers1D_{i:04d}.curve"
            butil.write_curve(fname, time, g.xi[0], eqs)
