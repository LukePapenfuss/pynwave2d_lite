...
class RK4:
    ...
    def step(self, e: Equations, g: Grid, dt, t):
        LTRACE = False
        ...
        for i in range(nu):
            us[i][:] = u0[i][:] + dt * k3[i][:]
        if e.apply_bc == BCType.FUNCTION:
            e.apply_bcs(us, g, t+dt)

        # Stage 4
        e.rhs(self.k4, us, g, t+dt)
        if self.do_rhs_filter:
            for i in range(nu):
                wrk = rhsfilter.filter(us[i])
                k4[i] += wrk
